...
function _addAnswerToTask(task,score,answer){task.answer=answer;task.score=score;return task;}
function _createTaskRun(score,answer,task){task=_addAnswerToTask(task,score,answer);var taskrun={'project_id':task.project_id,'task_id':task.id,'score':task.score,'info':task.answer};taskrun=JSON.stringify(taskrun);return _saveTaskRun(taskrun).then(function(data){return data;});}
...
pybossa.newTask=function(projectname){return _getProject(projectname).then(_getNewTask);};pybossa.saveTask=function(taskId,score=0,answer){if(typeof(taskId)==="number"){return _fetchTask(taskId).then(_createTaskRun.bind(undefined,score,answer));}
