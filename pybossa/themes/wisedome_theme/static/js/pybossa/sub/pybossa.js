...
(function(pybossa, $, undefined) {
    ...
    function _addAnswerToTask(task, score, answer) {
        task.answer = answer;
        task.score = score;
        return task;
    }

    function _createTaskRun(score, answer, task) {
        task = _addAnswerToTask(task, score, answer);
        var taskrun = {
    ...
    pybossa.saveTask = function (taskId, score=0, answer) {
        if (typeof(taskId) === "number") {
            return _fetchTask(taskId).then(_createTaskRun.bind(undefined, score, answer));
        }
