...
function CustomAlert(){
	...
	this.render = function(dialog){
		...
		if (value === '') {
			...
			document.getElementById('dialogboxbody').innerHTML ='<img src="/static/new_design/images/mail.svg" style="width: 10%;" >'+ '<h4>입력해주신 이메일로 인증번호가 전송되었습니다</h4>' + '<input type="text" id="auth_num" class="input-field" placeholder="인증번호를 입력하세요" style="background:#fff;">';


			document.getElementById('dialogOk').innerHTML = '<button type="button" onclick="Alert.ok()" id="email_check" class="btn-primary" style="width:48%; float: left;  margin-bottom:1rem;" >확 인</button>';
			//document.getElementById('dialogboxfoot').innerHTML = '<button type="button" id="email_check" class="btn-primary">확 인</button>';
...
function startTimer() {
	...
	/*if(m<0){document.getElementById('dialogtime').innerHTML= "인증시간이 만료되었습니다"}
	...
	document.getElementById('dialogResend').innerHTML = '<button type="button" onclick="resetTimer()" id="re-send-email"  class="btn-gradient-danger" style="width:48%; float: left; margin-right:0.5rem;">재전송</button>'; 
	document.getElementById('timer').innerHTML = `${m} : ${s}`;
	setTimeout(startTimer, 1000);
}
